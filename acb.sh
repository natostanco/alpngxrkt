...
acbuild --debug run -- /bin/sh -c "apk add --no-cache --update openssl nginx apr apr-util libjpeg-turbo icu pcre gnupg jq"
acbuild --debug run -- wget `curl -s https://api.github.com/repos/natostanco/ngxpgs/releases | grep browser_download_url | head -n 1 | cut -d '"' -f 4` -O ngx.tar.gz
